...
Workflow A — Synthetic data only (no downloads needed):
    python ml/train.py

Workflow B — With real crisis datasets (recommended, higher accuracy):
    ...
    # Step 2: train on the prepared CSV
    python ml/train.py --dataset ml/data/crisis_dataset.csv

─── What this script does ───────────────────────────────────────────────────
    1. Loads data from a prepared CSV (text, label) or falls back to built-in synthetic data
    2. Cleans and normalises text
    3. Splits 80/20 (stratified)
    4. Vectorizes with TF-IDF (unigrams + bigrams, 5,000 features)
    5. Balances with SMOTE (oversamples minority classes)
    6. Trains XGBoost
    7. Evaluates: accuracy, precision, recall, F1, false alarm rate
    8. Saves model.pkl and vectorizer.pkl
    9. Prints a full classification report
...
# ─── Built-in Synthetic Dataset (fallback when no prepared CSV is provided) ───
# Used when running `python ml/train.py` with no arguments.
# For real deployments, always prefer prepare_dataset.py + --dataset flag.
...
def clean_text(text: str) -> str:
    ...
    Strips URLs, mentions, hashtags, non-alpha characters, and extra whitespace.
    """
...
def load_prepared_dataset(dataset_path: Path) -> pd.DataFrame:
    """
    Loads a pre-prepared CSV produced by prepare_dataset.py.

    Expected columns: text, label
    Valid labels: critical, high, medium, low

    Args:
        dataset_path: Path to the prepared crisis_dataset.csv.
    ...
    if not dataset_path.exists():
        logger.error(
            ...
            '  python ml/prepare_dataset.py --source combined --input ml/data/ '
    ...
    if 'text' not in df.columns or 'label' not in df.columns:
        logger.error(
            'Prepared dataset must have "text" and "label" columns. Found: %s',
            list(df.columns),
        )
        sys.exit(1)

    df = df.rename(columns={'label': 'urgency'})
    valid = {'critical', 'high', 'medium', 'low'}
    df['urgency'] = df['urgency'].str.strip().str.lower()
    df = df[df['urgency'].isin(valid)]

    logger.info('Prepared dataset loaded from %s: %d samples.', dataset_path, len(df))
    return df[['text', 'urgency']]


def train(dataset_path: Optional[Path], output_dir: Path) -> None:
    """
    Full training pipeline from raw data to saved model artefacts.

    Args:
        dataset_path: Optional path to prepared CSV (from prepare_dataset.py).
                      If None, falls back to the built-in SYNTHETIC_SAMPLES.
        output_dir:   Directory where model.pkl, vectorizer.pkl, metadata.json are written.
    """
    import joblib
    ...
    if dataset_path is not None:
        df = load_prepared_dataset(dataset_path)
        logger.info('Using prepared dataset: %s', dataset_path)
    else:
        logger.info(
            'No --dataset provided. Using built-in synthetic samples (%d).\n'
            'For better accuracy, run:\n'
            '  python ml/prepare_dataset.py --source combined '
            '--input ml/data/ --output ml/data/crisis_dataset.csv\n'
            '  python ml/train.py --dataset ml/data/crisis_dataset.csv',
            len(SYNTHETIC_SAMPLES),
        )
        df = pd.DataFrame(SYNTHETIC_SAMPLES, columns=['text', 'urgency'])

    logger.info('Total samples before cleaning: %d', len(df))
    ...
    if len(df) < 20:
        logger.error('Not enough samples to train (%d). Add more data.', len(df))
        sys.exit(1)
    ...
    # Increase max_features to 10,000 when using large real datasets (CrisisLex/HumAID).
    # min_df=2 filters noise on large corpora; keep min_df=1 for small synthetic-only sets.
    n_samples = len(X_train_raw)
    min_df = 2 if n_samples >= 1000 else 1
    max_features = 10_000 if n_samples >= 5000 else 5000

    vectorizer = TfidfVectorizer(
        max_features=max_features,
        ngram_range=(1, 2),       # Unigrams + bigrams: "gas leak", "power outage"
    ...
    logger.info('TF-IDF: %d features (min_df=%d)', len(vectorizer.vocabulary_), min_df)

    # ── 6. SMOTE class balancing ──────────────────────────────────────────────
    # Critical/High alerts are naturally underrepresented in real datasets.
    # SMOTE synthesises minority class examples so the model does not default
    # to predicting Low for ambiguous inputs.
    if smote_available:
        ...
        if k >= 1:
            ...
            logger.warning(
                'SMOTE skipped: min class has only %d samples (need >= 2). '
                'Add more data or use --dataset with a larger combined CSV.',
                min_count,
            )

    # ── 7. Train XGBoost ──────────────────────────────────────────────────────
    # Hyperparameters are tuned for small-to-medium datasets.
    # With large real datasets (>10k samples): raise n_estimators to 400-500,
    # max_depth to 6, and lower min_child_weight to 1.
    n_train = len(y_train)
    n_estimators = 400 if n_train >= 2000 else 300
    max_depth = 6 if n_train >= 2000 else 4
    ...
    # PRD false alarm rate: low/medium misclassified as critical/high
    non_urgent_mask = y_test <= 1    # low=0, medium=1
    ...
    logger.info(
        'False Alarm Rate : %.2f%% (%d/%d non-urgent misclassified as urgent)',
    ...
    if not meets_accuracy:
        logger.warning(
            'Accuracy %.2f%% is below PRD target (80%%).\n'
            '  → Run prepare_dataset.py with --source combined and real CrisisLex/HumAID data.\n'
            '  → The model still works via keyword override for high-priority alerts.',
            accuracy * 100,
        )
    if not meets_false_alarm:
        logger.warning(
            'False alarm rate %.2f%% exceeds PRD limit (5%%).\n'
            '  → Add more low/medium training samples.\n'
            '  → Expand the keyword override list in alerts/services/classifier.py.',
    ...
    metadata = {
        ...
        'dataset_source': str(dataset_path) if dataset_path else 'synthetic_builtin',
    }
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Quickstart — synthetic data only:
  python ml/train.py

  # With real datasets (strongly recommended):
  python ml/prepare_dataset.py --source combined --input ml/data/ --output ml/data/crisis_dataset.csv
  python ml/train.py --dataset ml/data/crisis_dataset.csv
        """,
    )
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Path to prepared CSV from prepare_dataset.py (columns: text, label). '
             'If omitted, trains on built-in synthetic data.',
    )
    parser.add_argument(
        '--output-dir',
        type=Path,
    ...
    train(dataset_path=args.dataset, output_dir=args.output_dir)
