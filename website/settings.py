...
# Celery's default "prefork" concurrency pool uses os.fork(), which does not
# exist on Windows. On Windows, we fall back to "solo" (single-threaded,
# no child processes). In production (Linux), prefork runs normally.
# "solo" is fine for development — tasks run inline in the worker process.
import platform
