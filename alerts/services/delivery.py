...
def dispatch_alert(alert_id: str) -> dict:
    ...
    # ── Mark the alert as dispatched with recipient count ──────────────────────
    # Use .update() with F() rather than .save() to avoid race conditions and
    # to avoid rewriting every field of the row in a concurrent environment.
    from alerts.models import Alert as AlertModel
    AlertModel.objects.filter(id=alert.id).update(
        status=AlertModel.Status.DISPATCHED,
...
def _publish_to_redis(alert) -> bool:
    ...
    IMPORTANT: The payload shape must match the TypeScript Alert interface
    exactly (src/models/Alert.ts). The React Native websocketClient destructures
    the `type` field and passes the remainder directly to the onAlert callback,
    which expects a full Alert object. Any mismatch here means the alert arrives
    with undefined fields and the store/UI silently ignores it.
 
    Args:
        alert: Alert model instance (must have select_related('created_by') applied).
    ...
    try:
        ...
        # Build the payload to match the TypeScript Alert interface exactly.
        # The websocketClient strips the `type` key and passes the rest as an Alert.
        payload = json.dumps({
            # Routing key — consumed by websocketClient._handleMessage, stripped before callback
            'type': 'new_alert',
 
            # ── Full Alert shape (mirrors src/models/Alert.ts) ─────────────────
            'id': str(alert.id),
            'title': alert.title,
            'body': alert.body,
            'urgency': alert.urgency,
            ...
            # is_high_priority is a @property — include it for convenience but
            # the mobile client derives this from urgency itself
            'is_high_priority': alert.is_high_priority,
        })
