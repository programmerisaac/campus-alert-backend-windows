...
class DeliveryLogInline(admin.TabularInline):
    ...
    def get_queryset(self, request: HttpRequest) -> QuerySet:
        """
        Eagerly load users to avoid N+1.
        The slice is applied AFTER Django has filtered by the parent alert,
        using pk__in on a pre-evaluated list — safe from the 'slice then filter' error.
        """
        qs = (
            super()
    ...
    def get_max_num(self, request: HttpRequest, obj=None, **kwargs) -> int:
        """
        Allow no additions regardless of context.
        """
        return 0
...
class AlertAdmin(admin.ModelAdmin):
    ...
    def confidence_display(self, obj: Alert) -> str:
        ...
        if obj.classification_confidence is None:
            # mark_safe is correct here — this is a static string with no user data.
            # format_html() requires at least one substitution argument, so it
            # cannot be used for strings with no template variables.
